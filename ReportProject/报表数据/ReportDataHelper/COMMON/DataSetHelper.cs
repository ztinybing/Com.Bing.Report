...
namespace Com.Bing
{
    ...
    {
        public static DataTable DataRowArrayToDataTable(DataRow [] drs)
        {
        ...
        public DataTable Filt(DataTable sourceTable, string filt)
        {
            DataRow[] drs = sourceTable.Select(filt);
            DataTable destTable = sourceTable.Clone();
            for (int i = 0; i < drs.Length; i++)
            {
                destTable.Rows.Add(drs[i].ItemArray);
            }
            return destTable;
        }
